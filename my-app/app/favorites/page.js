...
const Favorites = () => {
    ...
    const { isloggedIn, isAuthLoaded, userEmail } = useAuth();
    ...
    async function fetchBlogs() {
        try {
            ...
            // Filter blogs that are favored
            const favIds = [];
            if (activeEmail) {
                try {
                    const favRes = await fetch(`/api/favorites?email=${activeEmail}`);
                    if (favRes.ok) {
                        const favData = await favRes.json();
                        favIds.push(...favData.blogIds);
                    }
                } catch(e) {}
            }
            const favBlogs = res.filter(b => favIds.includes(b._id));
            setBlogs(favBlogs);
            if (myblog.ok) {
                setLoading(false);
            }
    ...
    async function removeFavorite(blogId) {
        ...
        try {
            // Update Favorite model in DB
            await fetch('/api/favorites', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: activeEmail, blogId, action: 'unlike' })
            });
            
            // Decrease the actual likes count on standard toggle logic
        ...
        <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans ">
            ...
            <main className="flex-grow w-full max-w-7xl mx-auto px-6 py-12">
                <div className="flex items-center gap-4 mb-12">
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">Your Favorite Stories</h1>
                </div>
                ...
                {Loading ? (
                    ...
                    <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100 max-w-2xl mx-auto mt-12">
                        <div className="w-24 h-24 bg-rose-50 rounded-full flex items-center justify-center text-rose-300 mx-auto mb-6">
                            <i className="fa-regular fa-heart text-4xl"></i>
                        </div>
                        <h2 className="text-2xl font-bold text-gray-900 mb-4">No Favorites Yet</h2>
                        <p className="text-gray-500 mb-8 max-w-md mx-auto">You haven&apos;t favorited any stories yet. Start exploring and like the ones you love!</p>
